...
class _State extends ConsumerState<TeacherParentEngagementScreen> {
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor:AppTheme.stitchBg,appBar:AppBar(title:const Text('Parent Engagement'),actions:[IconButton(onPressed:()=>ref.invalidate(teacherParentContactsProvider),icon:const Icon(Icons.refresh_rounded))]),body:data.when(
      ...
      final list=ListView.separated(padding:const EdgeInsets.all(16),itemCount:items.length,separatorBuilder:(_, _)=>const SizedBox(height:8),itemBuilder:(context,i){final x=items[i];return Card(child:ListTile(onTap:()=>setState(()=>selected=x),leading:const CircleAvatar(child:Icon(Icons.family_restroom_rounded)),title:Text(x['parent_name']?.toString()??'Guardian'),subtitle:Text('${x['student_name']?.toString() ?? 'Student'} · ${x['subject_name']?.toString() ?? (x['is_homeroom']==true ? 'Homeroom' : 'Class')}'),trailing:const Icon(Icons.chevron_right_rounded)));});
